...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _DEVCON_PPS_LOCK_MX_VREG(index)
    _DEVCON_PMD_LOCK_MX_VREG(index)

  MASKs: 
    _DEVCON_PPS_LOCK_MX_MASK(index)
    _DEVCON_PMD_LOCK_MX_MASK(index)

  POSs: 
    _DEVCON_PPS_LOCK_MX_POS(index)
    _DEVCON_PMD_LOCK_MX_POS(index)

  LENs: 
    _DEVCON_PPS_LOCK_MX_LEN(index)
    _DEVCON_PMD_LOCK_MX_LEN(index)
...
PLIB_TEMPLATE void DEVCON_DeviceRegistersLock_PIC32MX( DEVCON_MODULE_ID index , DEVCON_REGISTER_SET registersToLock )
{
    ...
    {
        _SFR_BIT_SET(_DEVCON_PPS_LOCK_MX_VREG(index),_DEVCON_PPS_LOCK_MX_POS(index));
    }

    if ( 0 != (registersToLock & DEVCON_PMD_REGISTERS ))
    {
        _SFR_BIT_SET(_DEVCON_PMD_LOCK_MX_VREG(index),_DEVCON_PMD_LOCK_MX_POS(index));
    }
...
PLIB_TEMPLATE void DEVCON_DeviceRegistersUnlock_PIC32MX( DEVCON_MODULE_ID index , DEVCON_REGISTER_SET registersToLock )
{
    if ( 0 != (registersToLock & DEVCON_PPS_REGISTERS ))
    {
        _SFR_BIT_CLEAR(_DEVCON_PPS_LOCK_MX_VREG(index),_DEVCON_PPS_LOCK_MX_POS(index));
    }

    if ( 0 != (registersToLock & DEVCON_PMD_REGISTERS ))
    {
        _SFR_BIT_CLEAR(_DEVCON_PMD_LOCK_MX_VREG(index),_DEVCON_PMD_LOCK_MX_POS(index));
    }
