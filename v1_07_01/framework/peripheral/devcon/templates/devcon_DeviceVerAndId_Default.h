...
/* Routines available for accessing VREGS, MASKS, POS, LEN are
  ...
  VREGs:
    _DEVCON_DEVICE_REVISION_VREG(index)
    _DEVCON_DEVICE_ID_VREG(index)

  MASKs:
    _DEVCON_DEVICE_REVISION_MASK(index)
    _DEVCON_DEVICE_ID_MASK(index)

  POSs:
    _DEVCON_DEVICE_REVISION_POS(index)
    _DEVCON_DEVICE_ID_POS(index)

  LENs:
    _DEVCON_DEVICE_REVISION_LEN(index)
    _DEVCON_DEVICE_ID_LEN(index)
...
{
    return (uint8_t)_SFR_FIELD_READ(_DEVCON_DEVICE_REVISION_VREG(index),
                                    _DEVCON_DEVICE_REVISION_MASK(index),
                                    _DEVCON_DEVICE_REVISION_POS(index) );
}
...
{
    return (uint32_t)_SFR_FIELD_READ(_DEVCON_DEVICE_ID_VREG(index),
                                     _DEVCON_DEVICE_ID_MASK(index),
                                     _DEVCON_DEVICE_ID_POS(index) );
}
