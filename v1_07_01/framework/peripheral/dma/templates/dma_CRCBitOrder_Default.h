...
/* Routines available for accessing VREGS, MASKS, POS, LEN are
  ...
  VREGs:
    _DMA_CRC_BIT_ORDER_VREG(index)

  MASKs:
    _DMA_CRC_BIT_ORDER_MASK(index)

  POSs:
    _DMA_CRC_BIT_ORDER_POS(index)

  LENs:
    _DMA_CRC_BIT_ORDER_LEN(index)
...
{
        _SFR_BIT_WRITE(_DMA_CRC_BIT_ORDER_VREG(index),
                       _DMA_CRC_BIT_ORDER_POS(index) ,
                       bitOrder);
}
