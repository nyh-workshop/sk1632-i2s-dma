...
/* Routines available for accessing VREGS, MASKS, POS, LEN are
  ...
  VREGs:
    _DMA_CRC_CHANNEL_VREG(index)

  MASKs:
    _DMA_CRC_CHANNEL_MASK(index)

  POSs:
    _DMA_CRC_CHANNEL_POS(index)

  LENs:
    _DMA_CRC_CHANNEL_LEN(index)
...
{
        _SFR_FIELD_WRITE(_DMA_CRC_CHANNEL_VREG(index),
                         _DMA_CRC_CHANNEL_MASK(index),
                         _DMA_CRC_CHANNEL_POS(index) ,
                         channel);
}
...
{
        return (DMA_CHANNEL)_SFR_FIELD_READ(_DMA_CRC_CHANNEL_VREG(index),
                               _DMA_CRC_CHANNEL_MASK(index),
                               _DMA_CRC_CHANNEL_POS(index) );
}
