...
/* Routines available for accessing VREGS, MASKS, POS, LEN are
  ...
  VREGs:
    _DMA_CRC_DATA_VREG(index)

  MASKs:
    _DMA_CRC_DATA_MASK(index)

  POSs:
    _DMA_CRC_DATA_POS(index)

  LENs:
    _DMA_CRC_DATA_LEN(index)
...
{
	return _SFR_READ (_DMA_CRC_DATA_VREG(index));
}
...
{
		_SFR_WRITE (_DMA_CRC_DATA_VREG(index), DMACRCdata);
}
