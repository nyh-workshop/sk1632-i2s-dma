...
/* Routines available for accessing VREGS, MASKS, POS, LEN are
  ...
  VREGs:
    _DMA_CRC_TYPE_VREG(index)

  MASKs:
    _DMA_CRC_TYPE_MASK(index)

  POSs:
    _DMA_CRC_TYPE_POS(index)

  LENs:
    _DMA_CRC_TYPE_LEN(index)
...
{
    return (DMA_CRC_TYPE)_SFR_BIT_READ(_DMA_CRC_TYPE_VREG( index ), _DMA_CRC_TYPE_POS( index ));
}
...
{
        _SFR_BIT_WRITE(_DMA_CRC_TYPE_VREG(index),
                       _DMA_CRC_TYPE_POS(index) ,
                       CRCType                      );
}
