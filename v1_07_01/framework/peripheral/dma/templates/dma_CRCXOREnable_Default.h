...
/* Routines available for accessing VREGS, MASKS, POS, LEN are
  ...
  VREGs:
    _DMA_CRC_XOR_ENABLE_VREG(index)

  MASKs:
    _DMA_CRC_XOR_ENABLE_MASK(index)

  POSs:
    _DMA_CRC_XOR_ENABLE_POS(index)

  LENs:
    _DMA_CRC_XOR_ENABLE_LEN(index)
...
{
		_SFR_WRITE (_DMA_CRC_XOR_ENABLE_VREG(index), DMACRCXOREnableMask);
}
...
{
	return _SFR_READ (_DMA_CRC_XOR_ENABLE_VREG(index));
}
