...
/* Routines available for accessing VREGS, MASKS, POS, LEN are
  ...
  VREGs:
    _DMA_CHANNEL_X_ABORTIRQ_VREG(index)

  MASKs:
    _DMA_CHANNEL_X_ABORTIRQ_MASK(index)

  POSs:
    _DMA_CHANNEL_X_ABORTIRQ_POS(index)

  LENs:
    _DMA_CHANNEL_X_ABORTIRQ_LEN(index)
...
PLIB_TEMPLATE void DMA_ChannelXAbortIRQSet_Default( DMA_MODULE_ID index , DMA_CHANNEL channel , DMA_TRIGGER_SOURCE IRQ)
{
    ...
    pDmaChanXControlReg = (SFR_TYPE *)_DMA_CHANNEL_X_ABORTIRQ_VREG(index);
