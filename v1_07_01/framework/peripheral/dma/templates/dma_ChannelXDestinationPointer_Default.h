...
/* Routines available for accessing VREGS, MASKS, POS, LEN are
  ...
  VREGs:
    _DMA_CHANNEL_X_DESTINATIONPOINTER_VREG(index)

  MASKs:
    _DMA_CHANNEL_X_DESTINATIONPOINTER_MASK(index)

  POSs:
    _DMA_CHANNEL_X_DESTINATIONPOINTER_POS(index)

  LENs:
    _DMA_CHANNEL_X_DESTINATIONPOINTER_LEN(index)
...
PLIB_TEMPLATE uint16_t DMA_ChannelXDestinationPointerGet_Default( DMA_MODULE_ID index , DMA_CHANNEL dmaChannel )
{
    ...
    pDmaChanXControlReg = (SFR_TYPE *)_DMA_CHANNEL_X_DESTINATIONPOINTER_VREG(index);
