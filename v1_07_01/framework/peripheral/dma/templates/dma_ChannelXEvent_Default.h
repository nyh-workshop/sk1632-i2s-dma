...
/* Routines available for accessing VREGS, MASKS, POS, LEN are
  ...
  VREGs:
    _DMA_CHANNEL_X_EVENT_VREG(index)

  MASKs:
    _DMA_CHANNEL_X_EVENT_MASK(index)

  POSs:
    _DMA_CHANNEL_X_EVENT_POS(index)

  LENs:
    _DMA_CHANNEL_X_EVENT_LEN(index)
...
PLIB_TEMPLATE bool DMA_ChannelXEventIsDetected_Default( DMA_MODULE_ID index , DMA_CHANNEL channel )
{
    ...
    pDmaChanXControlReg = (SFR_TYPE *)_DMA_CHANNEL_X_EVENT_VREG(index);

    return 	_SFR_BIT_READ(&pDmaChanXControlReg[48*channel], _DMA_CHANNEL_X_EVENT_POS( index ) );
