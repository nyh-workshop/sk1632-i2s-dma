...
/* Routines available for accessing VREGS, MASKS, POS, LEN are
  ...
  VREGs:
    _DMA_RECENTADDRESS_VREG(index)

  MASKs:
    _DMA_RECENTADDRESS_MASK(index)

  POSs:
    _DMA_RECENTADDRESS_POS(index)

  LENs:
    _DMA_RECENTADDRESS_LEN(index)
...
{
	return (uint32_t) ConvertToVirtualAddress(_SFR_READ (_DMA_RECENTADDRESS_VREG(index)));
}
