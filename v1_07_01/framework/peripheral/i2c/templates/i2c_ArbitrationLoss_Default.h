...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _I2C_ARBITRATION_LOSS_VREG(index)

  MASKs: 
    _I2C_ARBITRATION_LOSS_MASK(index)

  POSs: 
    _I2C_ARBITRATION_LOSS_POS(index)

  LENs: 
    _I2C_ARBITRATION_LOSS_LEN(index)
...
{
    return _SFR_BIT_READ(_I2C_ARBITRATION_LOSS_VREG(index),
                             _I2C_ARBITRATION_LOSS_POS(index) );
}
...
{
    _SFR_BIT_CLEAR(_I2C_ARBITRATION_LOSS_VREG(index),
                       _I2C_ARBITRATION_LOSS_POS(index) );
}
