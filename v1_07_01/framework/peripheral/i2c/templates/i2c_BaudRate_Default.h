...
//  				(PULSE_GOBBLER_DELAY/1000000.0f)) * clockFrequency) - 2 ));
	...
	_SFR_WRITE(_I2C_BAUD_RATE_VREG(index), ((clockFrequency / (2 * baudRate)) - (clockFrequency / 10000000) - 2) );
}
...
//            (clockFrequency*(PULSE_GOBBLER_DELAY/1000000.0f)) +2 ));
	  return (((clockFrequency /((_SFR_READ(_I2C_BAUD_RATE_VREG(index))+2) + (clockFrequency / 10000000)))) >> 1);
