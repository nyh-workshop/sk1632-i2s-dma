...
/* Routines available for accessing VREGS, MASKS, POS, LEN are
  ...
  VREGs:
    _OSC_OSC_PB_DIVISOR_VREG(index)
    _OSC_OSC_REGISTER_LOCK_VREG(index)

  MASKs:
    _OSC_OSC_PB_DIVISOR_MASK(index)
    _OSC_OSC_REGISTER_LOCK_MASK(index)

  POSs:
    _OSC_OSC_PB_DIVISOR_POS(index)
    _OSC_OSC_REGISTER_LOCK_POS(index)

  LENs:
    _OSC_OSC_PB_DIVISOR_LEN(index)
...
{
    return (0x01 << _SFR_FIELD_READ(_OSC_OSC_PB_DIVISOR_VREG(index),
                           _OSC_OSC_PB_DIVISOR_MASK(index),
                           _OSC_OSC_PB_DIVISOR_POS(index) ));
}
...
PLIB_TEMPLATE void OSC_PBClockDivisorSet_PIC32_1( OSC_MODULE_ID index , OSC_PERIPHERAL_BUS peripheralBusNumber , OSC_PB_CLOCK_DIV_TYPE peripheralBusClkDiv )
{
	...
	for (regValue=0; regValue<4; regValue++)
	{
		if ( peripheralBusClkDiv == ( 0x01<<regValue ) )
		{
			_SFR_FIELD_WRITE(_OSC_OSC_PB_DIVISOR_VREG(index),
							 _OSC_OSC_PB_DIVISOR_MASK(index),
							 _OSC_OSC_PB_DIVISOR_POS(index) ,
								 regValue );
			return;
