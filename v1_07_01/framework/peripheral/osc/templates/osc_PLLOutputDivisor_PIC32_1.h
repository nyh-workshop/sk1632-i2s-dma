...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _OSC_OSC_OUTPUT_DIVISOR_VREG(index)
    _OSC_OSC_REGISTER_LOCK_VREG(index)

  MASKs: 
    _OSC_OSC_OUTPUT_DIVISOR_MASK(index)
    _OSC_OSC_REGISTER_LOCK_MASK(index)

  POSs: 
    _OSC_OSC_OUTPUT_DIVISOR_POS(index)
    _OSC_OSC_REGISTER_LOCK_POS(index)

  LENs: 
    _OSC_OSC_OUTPUT_DIVISOR_LEN(index)
...
{
    _SFR_FIELD_WRITE(_OSC_OSC_OUTPUT_DIVISOR_VREG(index),
                     _OSC_OSC_OUTPUT_DIVISOR_MASK(index),
                     _OSC_OSC_OUTPUT_DIVISOR_POS(index) ,
                     PLLOutDiv );
}
...
PLIB_TEMPLATE uint16_t OSC_SysPLLOutputDivisorGet_PIC32_1( OSC_MODULE_ID index )
{
    ...
    regValue = _SFR_FIELD_READ(_OSC_OSC_OUTPUT_DIVISOR_VREG(index),
                           _OSC_OSC_OUTPUT_DIVISOR_MASK(index),
                           _OSC_OSC_OUTPUT_DIVISOR_POS(index) );
