...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _SPI_BAUD_RATE_VREG(index)

  MASKs: 
    _SPI_BAUD_RATE_MASK(index)

  POSs: 
    _SPI_BAUD_RATE_POS(index)

  LENs: 
    _SPI_BAUD_RATE_LEN(index)
...
PLIB_TEMPLATE void SPI_BaudRateSet_Default( SPI_MODULE_ID index , uint32_t clockFrequency , uint32_t baudRate )
{
	...
	t_brg = (((clockFrequency/baudRate)/2) - 1);
	
    baudHigh = clockFrequency / (2 * (t_brg + 1));
    baudLow = clockFrequency / (2 * (t_brg + 2));
	...
	_SFR_FIELD_WRITE( _SPI_BAUD_RATE_VREG( index ),
				  _SPI_BAUD_RATE_MASK( index ),
				  _SPI_BAUD_RATE_POS( index ),
				  t_brg );
}
