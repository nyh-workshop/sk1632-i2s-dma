...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _SPI_OUTPUT_DATA_PHASE_VREG(index)

  MASKs: 
    _SPI_OUTPUT_DATA_PHASE_MASK(index)

  POSs: 
    _SPI_OUTPUT_DATA_PHASE_POS(index)

  LENs: 
    _SPI_OUTPUT_DATA_PHASE_LEN(index)
...
PLIB_TEMPLATE void SPI_OutputDataPhaseSelect_Default( SPI_MODULE_ID index , SPI_OUTPUT_DATA_PHASE phase )
{
    switch( phase )
    {
        case SPI_OUTPUT_DATA_PHASE_ON_ACTIVE_TO_IDLE_CLOCK:
                _SFR_BIT_SET( _SPI_OUTPUT_DATA_PHASE_VREG( index ),
                              _SPI_OUTPUT_DATA_PHASE_POS( index ) );
            break;
        case SPI_OUTPUT_DATA_PHASE_ON_IDLE_TO_ACTIVE_CLOCK:
                _SFR_BIT_CLEAR( _SPI_OUTPUT_DATA_PHASE_VREG( index ),
                                _SPI_OUTPUT_DATA_PHASE_POS( index ) );
            break;
        default:
            break;
